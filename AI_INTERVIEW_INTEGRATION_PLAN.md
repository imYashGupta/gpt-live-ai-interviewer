...
3. Set `INTERVIEW_SERVICE_TEAM_IDS` to explicit internal team IDs, then enable the feature. Keep `INTERVIEW_SERVICE_SANDBOX=true` and `INTERVIEW_SERVICE_ALLOW_INVITATIONS=false` while testing link generation. A team workspace is provisioned lazily using stable request keys. Accounts and existing workspace mappings cannot silently be swapped.
...
- Existing known interview mappings plus idempotent create commands and per-interview polling cover this slice. A cursor-based interview listing is not required for these observed gaps; it remains deferred for broader discovery/reconciliation. Canonical service contracts and the pinned ATS snapshot did not change.
...
**Delivered:** the local report-delivery and recovery acceptance slice. **Still open:** actual signed webhook delivery to an approved public HTTPS receiver, deployed outage acceptance, ownership challenge/signing-key rotation and broader lifecycle/privacy/billing gates. Local Recooty remains HTTP; local signed-request tests and injected transport tests do not prove public delivery. HTTPS/public-address restrictions remain intact. The full Phase 3 production exit gate is not complete.
