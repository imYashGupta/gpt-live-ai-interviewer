...
export async function migrateService(pool: Pool): Promise<void> {
  for (const [version, file] of [[1, "001_foundations.sql"], [2, "002_live_execution.sql"], [3, "003_interview_deletion.sql"]] as const) {
    ...
    await transaction(pool, async (db) => {
      ...
      const previous = await db.query("SELECT checksum FROM service_migrations WHERE version=$1", [version]);
      if (previous.rowCount) {
        if (previous.rows[0].checksum !== hash) throw new Error("Applied migration checksum changed");
        return;
      }
      await db.query(sql);
      await db.query("INSERT INTO service_migrations(version,checksum) VALUES ($1,$2)", [version, hash]);
    });
  }
}
